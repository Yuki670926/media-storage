...
**推奨: まず自宅マシン(または NAS)+ ローカルファイルシステムで開始し、ストレージ層は最初からインターフェースで抽象化しておく。** 後から S3 互換(MinIO / B2)に差し替えたり、二次バックアップ先として追加したりできます。具体的な機材の比較・選定と段階的な拡張プランは [HARDWARE.md](HARDWARE.md) 参照。
...
### 2.5 セキュリティ(要件: REQUIREMENTS.md NFR-4)

外出先からの閲覧は確定要件。ただし「インターネットに公開」しなくても実現できる:

- **方式A(既定・推奨): Tailscale。** サーバーと自分の iPhone/PC を同じ VPN(tailnet)に入れる。外出先からも自分の端末なら閲覧でき、インターネットには一切公開しない(攻撃面ゼロ)。証明書・NAT 越えも Tailscale が処理するため、CGNAT 回線(v6 プラス等)でも動く。
- **方式B(任意): リバースプロキシ公開。** Caddy + Let's Encrypt。NFR-4 の硬化条件(強認証・レート制限・管理系エンドポイント非公開・依存更新の継続)をすべて満たす場合のみ。
- **全メディア・メタデータ API は認証必須。未認証で 1 バイトも返さない。** オリジナルには GPS 付き EXIF(= 自宅位置が特定できる PII)が含まれるため、URL の推測不能性(UUID)だけに頼らない。
- 認証はデバイスごとの **API トークン**(初回ペアリングで発行、サーバー側で 1 台単位で失効可能、DB にはハッシュのみ保存)。マルチユーザーは対象外(スキーマの `user_id` のみ温存)。
- E2E 暗号化は**不採用**(サーバー側サムネイル生成・出口戦略と両立しないため)。
...
- **iPhone 側の写真を消さない(当面の運用ルール)**: オフサイトコピーは当面見送り(REQUIREMENTS.md NFR-1 の受容リスク)のため、iPhone/iCloud が 2 コピー目の役割を担う。iPhone から削除を始めたくなったら、先に 2 台目ディスクの日次ミラー([HARDWARE.md](HARDWARE.md) Stage 1)かオフサイトを導入すること。
- **バックアップのバックアップ**: 将来オフサイトを導入する際は `originals/` + DB ダンプを rclone(暗号化)で別媒体へ送る。バックアップ対象はこの 2 つだけでよい設計になっている。
- **容量見積り**: ライブラリは 200 GB〜で継続増加(確定)。ディスクは 4 TB 開始を推奨。機材選定と拡張プランは [HARDWARE.md](HARDWARE.md)。
